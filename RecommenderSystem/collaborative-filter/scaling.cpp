...
/**
 * An implementation of the recommendation algorithm Scaling the scales
 * 
 ...
 */
Scaling::Scaling(Dataset const &dataset, int const itemsInCommon, double const fracInCommon, double const maxDiff, int const maxDist)
: m_itemsInCommon(itemsInCommon)
, m_fracInCommon(fracInCommon)
, m_maxDiff(maxDiff)
, CollaborativeFilter("scaling", dataset, *new InverseScaledInvertible(dataset.scaleSize, *new Average()), *new ConstantDistance(maxDist)) {
 ...
 * @param maxDiff Maximum average difference for the transformation functions t_diff_max
 */
void Scaling::relate(int const itemsInCommon, double const fracInCommon, double const maxDiff) {
   relateRanges
 ...
 */
bool Scaling::commonality(int const predictor, int const predicted, int const ratings) const {
 ...
 * @return Whether the users can predict each other
 */
bool Scaling::commonality(int const predicted, int const ratings, int const itemsInCommon, double const fracInCommon) const {
 ...
 * @return Whether the users can predict each other
 */
bool Scaling::rangeCommonality(int const predictor, int const predicted, int const ratings, int const range) const {
   if(-1 == range)
 ...
 * @return Whether the users can predict each other
 */
bool Scaling::compatibility(double const coeff, double const maxDiff) const {
 ...
 */
std::string Scaling::toString(int const itemsInCommon, double const fracInCommon, double const maxDiff) const {
 ...
 */
std::string Scaling::rangeToString(int const range) const {
