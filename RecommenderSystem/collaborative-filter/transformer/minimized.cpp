...
TransformerMinimized::~TransformerMinimized() {
   delete &m_minimizer;
 ...
 */
TransformerMinimized::TransformerMinimized(int const scaleSize, TransformerMinimizer &minimizer) : m_minimizer(minimizer), Transformer(scaleSize) {
   reset();
} // TransformerMinimized
 ...
 */
void TransformerMinimized::reset() {
   m_diff = 0;
   m_bestDiff = -1;
} // reset
 ...
 */
void TransformerMinimized::report(double const d) {
   m_diff += d;
} // report
 ...
 */
double TransformerMinimized::getCoeff() const {
   return m_bestDiff;
} // getCoeff
 ...
 */
void TransformerMinimized::run(int const user1, int const user2, std::vector<RatingPair> const &ratings) {
   for(reset(); !done(); next()) {
